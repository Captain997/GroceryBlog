<template>
    <div class="websiteInfo">
        ...
        <ul>
            <li><b>建站时间：</b>2023年3月10日</li>
            <li><b>采用技术：</b>Java、Vue3</li>
            <li><b>文章统计：</b>{{articleCount}} 篇文章</li>
            <li><b>本站走过：</b>{{distance}}</li>
            <li><b>微信公众号：</b></li>
            <li class="websiteInfo_img">
                <img src="../../../../assets/weChat_tencent.jpg" alt="">
            </li>
        </ul>
    </div>
</template>
...
<script setup>
    ...
    axios.get("/article/showArticleCount").then(res => {
        articleCount.value = res.data < 10 ? "0" + res.data : res.data
    })

    const distance = ref('00 天 00 时 00 分 00 秒');
    setInterval(() => {
        var day = Math.floor(new Date().getTime() / 1000) - (new Date('2023/03/10 00:00:00').getTime() / 1000),
